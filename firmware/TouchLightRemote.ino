...
*/
	if ((serverColorAndTouch != lastServerColorAndTouch) &&	(deviceId != myId) && (deviceId != 0)) { // server update
		serverAndLocalDifferentLoopCount = 0;
		if (D_SERIAL){
			Serial.println("SERVER -> LOCAL");
		} 
		...
		if 	(((serverColorAndTouch & 0xff) != (lastServerColorAndTouch & 0xff)) && (serverTouch == tEVENT_RELEASE)) { // &&
